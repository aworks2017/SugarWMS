...
{if $hideTable == false}
	...
	href="index.php?module={$linkModule}&offset={$offset}&stamp={$pageData.stamp}&return_module={$linkModule}&action={$action}&record={$rowData.ID}"
				...
				{foreach from=$displayColumns key=col item=params}
					...
					<td {if $scope_row} scope='row' {/if} align='{$params.align|default:'left'}' valign="top" class="{if ($params.type == 'teamset')}nowrap{/if}{if preg_match('/PHONE/', $col)} phone{/if}">
					   ...
					   {if $params.link && !$params.customCode && $params.name neq 'assigned_user_name'}
	...
	{capture assign=url}index.php?module={if $linkModule=='queue'}jobs{else}{$linkModule}{/if}&offset={$offset}&stamp={$pageData.stamp}&return_module={$linkModule}&action={$action}&record={$record}{/capture}
	                        <{$pageData.tag.$id[$params.ACLTag]|default:$pageData.tag.$id.MAIN} href="{sugar_ajax_url url=$url}">
						{/if}
						
						{if  $params.name eq 'assigned_user_name'}
						<span assigned-user-id="{$rowData[$params.id]}" id="{$rowData.ID}" class='assigned_user_id'>
						{/if}
						...
						{/if}
						{if empty($rowData.$col) && empty($params.customCode)&& $params.name eq 'assigned_user_name'}--------{/if}
						{if  $params.name eq 'assigned_user_name'}
						</span>
						{/if}
						{if empty($rowData.$col) && empty($params.customCode)&& $params.name neq 'assigned_user_name'}&nbsp;{/if}
						{if $params.link && !$params.customCode && $params.name neq 'assigned_user_name'}
