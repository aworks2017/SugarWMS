...
class DynamicField {
  ...
  function getJOIN( $expandedList = false , $includeRelates = false, &$where = false){
   ...
   function getRelateJoin($field_def, $joinTableAlias, $withIdName = true) {
        ...
        {
			$rname = !empty($field_def['rname']) ? $field_def['rname'] : 'name';
            ...
            else if(!empty($rel_mod->field_defs[$rname]['source']) && $rel_mod->field_defs[$rname]['source'] == "non-db" && !empty($field_def['rname']))
            {
                $name_field = "$joinTableAlias." . $field_def['rname'];
            }
            else
            {
                $name_field = "$joinTableAlias.{$rname}";
            }
        }
