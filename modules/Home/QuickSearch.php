        ...
        foreach ($args['modules'] as $module) {
            ...
            if ($focus->ACLAccess('ListView', true)) {
				...
				if(isset($args['account_id']) && !empty($args['account_id'])){
					...
					$accounts->retrieve($args['account_id']); 
					$projects = $accounts->get_linked_beans('project','Project'); 
					$project_ids = array();
					$project_id = array();
					...
					where project.deleted=0 AND project.id IN('".implode("','",$project_ids)."')";
					$result_lead = $db->query($sql, true);
					while($row = $db->fetchByAssoc($result_lead)){
						$project_id[]= $row['id'];
					}
					$where .=" AND project.id IN('".implode("','",$project_id)."')";
				}
				if($args['populate_list'][0] == 'project_name_basic' || $args['populate_list'][0] == 'project_name_advanced'){
					$where .=" AND project.status='Published' ";
				}
				if($args['populate_list'][0] == 'account_name_basic' || $args['populate_list'][0] == 'account_name_advanced'){
					$where .=" AND accounts_cstm.custom_status_c='active' ";
				}
