...
require_once('modules/jobs/updateQueue.php');
...
class ImportViewConfirm extends ImportView
{
 	...
 	public function display()
    {
		...
		if($_REQUEST['import_module'] ==  'jobs'){
			$this->pageTitleKey = 'LBL_STEP_5_TITLE';
		}
		...
		if($_REQUEST['import_module'] ==  'jobs'){
			$i= 0;
			$fp = fopen($uploadFileName.'_temp', "w");
			foreach($importFile as  $row)
			{
				if($i==0){
					...
					$project->retrieve($row[3]); 
					$projecttasks = $project->get_linked_beans('projecttask','ProjectTask');
					...
					foreach ($projecttasks as $pt) {
						...
						$jobs[1] = $row[3];
						if(empty($row[4])){
							$jobs[2] = $project->parameter_1;
						}else{
							$jobs[2] = $row[4];
						}
						if(empty($row[5])){
							$jobs[3] = $project->parameter_2;
						}else{
							$jobs[3] = $row[5];
						}
						if($row[6] == ""){
							$jobs[4] = $pt->default_count_c;
						}else{
							$jobs[4] = $row[6];
						}
						$jobs[5] = $pt->name;
						//Default assigned_user_id as Automation Queue (154bb81c-8b80-bd12-9605-56836966fac3)
						$jobs[6] = $row[7];
						$jobs[7] = '';
						$jobs[8] = $pt->production_notes;
						$jobs[9] = $pt->id;
						$jobs[10] = $pt->estimated_effort;
						if($row[8] == ""){
							$jobs[11] = $project->restrict_start_days;
						}else{
							$jobs[11] = $row[8];
						}
						$data = implode(',', $jobs)."\n";
			...
			fclose($fp);
			$uploadFile_temp = $uploadFileName."_temp" ;
		}
		...
		if($_REQUEST['import_module'] ==  'jobs'){
			...
			$_REQUEST['columncount'] = '13';
			$_REQUEST['importlocale_charset'] = 'UTF-8';
			$_REQUEST['colnum_0'] = 'account_id';
			$_REQUEST['colnum_1'] = 'project_id';
			$_REQUEST['colnum_2'] = 'parameter_1';
			$_REQUEST['colnum_3']='parameter_2'; 
			$_REQUEST['colnum_4']='activity_count'; 
			$_REQUEST['colnum_5']='name';
			$_REQUEST['colnum_6']='project_due_date';
			$_REQUEST['colnum_7']='assigned_user_id';
			$_REQUEST['colnum_8']='production_notes';
			$_REQUEST['colnum_9']='project_task_id';
			$_REQUEST['colnum_10']='estimated_mins';
			$_REQUEST['colnum_11']='restrict_start_days';
			$_REQUEST['importlocale_dateformat'] = $dateFormat;
           	$_REQUEST['importlocale_timeformat'] = $timeFormat;
           	$_REQUEST['importlocale_timezone'] = $this->getDateTimeProperties($importFileMap);;
			$_REQUEST['has_header'] = 'on';
			$_REQUEST['firstrow'] = base64_encode(serialize($rows[0]));
			// Open the temp import file
			$importSource = new ImportFile($uploadFile_temp, $_REQUEST['custom_delimiter'],html_entity_decode($_REQUEST['custom_enclosure'],ENT_QUOTES));
	...
	private function getDateTimeProperties( $field_map = array() )
    {
