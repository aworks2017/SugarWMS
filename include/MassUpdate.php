...
eoq;
	...
	function handleMassUpdate(){
		...
		elseif(isset($_REQUEST['entire']) && empty($_POST['mass'])) {
			...
			if (strpos($this->where_clauses, 'jobs.start_date_before') !== false) {
				global $db, $timedate, $current_user;
				$request_data = unserialize(base64_decode($_REQUEST['current_query_by_page']));
				$start_date = $request_data['start_date_before_basic'];
				...
				$cstm_where = explode('AND', $this->where_clauses);
				...
				$this->where_clauses = $add_where.implode("AND",$cstm_where);
				if(!empty($cstm_where)) $this->where_clauses.=" AND";
				$this->where_clauses .=" (jobs.restrict_start_days='' OR jobs.restrict_start_days IS NULL OR jobs.id IN ('{$job_ids_where}')) ";
			}
			if(isset($_REQUEST['module']) && $_REQUEST['module'] == 'queue'){
				if(!empty($where_clauses)) $where_clauses .=' AND ';
				$where_clauses .= " jobs.status IN('Ready', 'Not Ready', 'Started', 'In Progress', 'Issue','Defer','On Hold') ";
				$where_clauses .= " AND jobs.order_number IS NOT NULL AND jobs.order_number !=0 ";
			}
            $query = $this->sugarbean->create_new_list_query($order_by, $this->where_clauses, array(), array(), 0, '', false, $this, true, true);
			$query = str_replace("queue", "jobs", $query);
				...
				if(isset($field['type']))
				{
					switch($field["type"])
					{
						...
						$even = !$even;
							...
							$newhtml .= $this->addStatus($displayname,  $field["name"], $this->getAssociates('e74a4e89-d82c-30d3-9277-54c187e227fb'));
							break;
...
EOJS;
	...
	public function getAssociates($role_id){
		global $db;
		$sql = "SELECT u.id, CONCAT_WS(' ', u.first_name, u.last_name) AS name
				FROM acl_roles ar
				INNER JOIN acl_roles_users aru ON(aru.deleted=0 AND ar.id=aru.role_id)
				INNER JOIN users u ON(u.deleted=0 AND u.id=aru.user_id)
				WHERE ar.deleted=0 AND u.status='Active' AND ar.id = '{$role_id}'
