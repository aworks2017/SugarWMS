...
require_once('include/EditView/EditView2.php');
 ...
 class SearchForm extends EditView{
    ...
    public function SearchForm($seed, $module, $action = 'index', $options = array())
    {
		...
		if(empty($_REQUEST['account_type_basic'][0])) unset($_REQUEST['account_type_basic']);
		if(empty($_REQUEST['custom_status_c_basic'][0]) || $_REQUEST['custom_status_c_basic'][0] =='undefined') unset($_REQUEST['custom_status_c_basic']);
		if(empty($_REQUEST['assigned_user_id_advanced'][0])) unset($_REQUEST['assigned_user_id_advanced']);
		if(empty($_REQUEST['account_type_advanced'][0])) unset($_REQUEST['account_type_advanced']);
		if(empty($_REQUEST['industry_advanced'][0])) unset($_REQUEST['industry_advanced']);
		if(empty($_REQUEST['custom_status_c_advanced'][0]) || $_REQUEST['custom_status_c_advanced'][0] =='undefined') unset($_REQUEST['custom_status_c_advanced']);
		if($_REQUEST['module'] == 'Project' && (empty($_REQUEST['status_basic'][0]) || $_REQUEST['status_basic'][0] == 'undefined')) unset($_REQUEST['status_basic']);
