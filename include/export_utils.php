...
function export($type, $records = null, $members = false, $sample=false) {
    ...
    if($records) {
        ...
        if(!empty($_REQUEST['current_post'])) {
			$request_data = unserialize(base64_decode($_REQUEST['current_post']));
			if(isset($request_data['have_notes_advanced'])){
				unset($request_data['have_notes_advanced']);
			}
			$_REQUEST['current_post'] = base64_encode(serialize($request_data));
	...
	if (strpos($where, 'jobs.start_date_before') !== false) {
		global $db,$timedate;
		$request_data = unserialize(base64_decode($_REQUEST['current_post']));
		$start_date = $request_data['start_date_before_basic'];
		$db_start_date = $timedate->to_db_date($start_date, false);
		...
		$where = $add_where.implode("AND",$cstm_where);
		if(!empty($cstm_where)) $where.=" AND";
		$where .=" (jobs.restrict_start_days='' OR jobs.restrict_start_days IS NULL OR jobs.id IN ('{$job_ids_where}')) ";
	}
	if(isset($_REQUEST['module']) && $_REQUEST['module'] == 'queue'){
		if(!empty($where)) $where .=' AND ';
		$where .= " jobs.status IN('Ready', 'Not Ready', 'Started', 'In Progress', 'Issue','Defer','On Hold') ";
        $where .= " AND jobs.order_number IS NOT NULL AND jobs.order_number !=0 ";
	}
    if($members == true){
